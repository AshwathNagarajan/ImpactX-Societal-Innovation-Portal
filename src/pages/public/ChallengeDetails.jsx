...
export default function ChallengeDetails() {
  ...
  return <div className="mx-auto max-w-7xl px-4 py-6 sm:py-8">
    <div className="rounded-2xl border bg-white p-4 shadow-sm sm:p-6"><div className="flex flex-wrap justify-between gap-3"><div><p className="text-sm font-semibold text-blue">{c.id}</p><h1 className="mt-1 text-3xl font-semibold leading-tight text-navy sm:text-4xl">{c.title}</h1></div><div className="flex gap-2"><StatusBadge status={c.status}/><PriorityBadge priority={c.priority}/></div></div><p className="mt-4 max-w-4xl text-sm leading-6 text-slate-600 sm:text-base">{c.description}</p><div className="mt-5 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4"><Info k="Category" v={c.category}/><Info k="Location" v={`${c.city}, ${c.district}`}/><Info k="Submitted" v={c.date}/><Info k="People affected" v={Number(c.affected).toLocaleString()}/><Info k="Submitted by" v={c.submitter}/><Info k="Priority" v={c.priority}/></div></div>
    <section className="mt-6 rounded-2xl border bg-white p-4 shadow-sm sm:p-5"><h2 className="mb-4 text-xl font-semibold text-navy sm:text-2xl">Status Timeline</h2><Timeline steps={steps}/></section>
