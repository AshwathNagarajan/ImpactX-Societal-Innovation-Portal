...
export default function ChallengeDetails() {
  ...
  return <div className="mx-auto max-w-7xl px-4 py-6 sm:py-8">
    <div className="rounded-2xl border bg-white p-4 shadow-sm md:p-6"><div className="flex flex-wrap justify-between gap-3"><div className="min-w-0"><p className="text-sm font-semibold text-blue">{c.id}</p><h1 className="mt-1 text-3xl font-semibold leading-tight text-navy md:text-4xl">{c.title}</h1></div><div className="flex flex-wrap gap-2"><StatusBadge status={c.status}/><PriorityBadge priority={c.priority}/></div></div><p className="mt-4 max-w-4xl text-sm leading-6 text-slate-600 md:text-base">{c.description}</p><div className="mt-5 grid gap-3 text-sm md:grid-cols-2 lg:grid-cols-4"><Info k="Category" v={c.category}/><Info k="Location" v={`${c.city}, ${c.district}`}/><Info k="Submitted" v={c.date}/><Info k="People affected" v={Number(c.affected).toLocaleString()}/><Info k="Submitted by" v={c.submitter}/><Info k="Priority" v={c.priority}/></div></div>
    <section className="mt-6 rounded-2xl border bg-white p-4 shadow-sm md:p-5"><h2 className="mb-4 text-xl font-semibold text-navy md:text-2xl">Status Timeline</h2><Timeline steps={steps}/></section>
    <div className="mt-6 grid gap-6 lg:grid-cols-[.9fr_1.1fr]">
      <section className="rounded-xl border bg-white p-5 shadow-sm"><h2 className="text-xl font-semibold text-navy md:text-2xl">AI Analysis</h2><div className="mt-4 space-y-3 text-sm"><Info k="Suggested category" v={c.category}/><Info k="Priority score" v={`${c.priority==="Critical"?94:c.priority==="High"?86:68}/100`}/><Info k="Duplicate probability" v="12% low-risk match"/><Info k="Recommended institute domains" v="AI, IoT, field research, public systems"/><Info k="Required expertise" v="Human-centered design, sensors, data analytics"/><Info k="Impact score" v="88/100"/></div></section>
      <section className="min-w-0 rounded-xl border bg-white p-5 shadow-sm"><h2 className="text-xl font-semibold text-navy md:text-2xl">Impact Metrics</h2><ResponsiveContainer width="100%" height={240}><BarChart data={[{name:"Reach",value:c.affected},{name:"Households",value:Math.round(c.affected/4.8)},{name:"Pilot users",value:Math.round(c.affected*.18)}]}><XAxis dataKey="name"/><YAxis/><Tooltip/><Bar dataKey="value" fill="#14B8A6" radius={[6,6,0,0]}/></BarChart></ResponsiveContainer></section>
    </div>
...
function Grid({ title, children }) { return <section className="mt-6"><h2 className="mb-4 text-xl font-semibold text-navy md:text-2xl">{title}</h2><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{children}</div></section>; }
