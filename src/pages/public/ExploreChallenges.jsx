...
export default function ExploreChallenges() {
  ...
  return <div className="mx-auto max-w-7xl px-4 py-8"><span className="rounded-full border border-blue/20 bg-blue/10 px-3 py-1 text-sm font-semibold text-blue">Challenge Explorer</span><h1 className="mt-4 text-4xl font-semibold tracking-tight text-navy">Explore Societal Challenges</h1><p className="mt-2 text-slate-600">Discover real problems waiting for innovative solutions.</p><div className="mt-6 sticky top-[76px] z-10"><SearchFilter filters={filters} setFilters={setFilters}/></div><p className="mt-5 text-sm text-slate-500">Showing <span className="text-navy">{rows.length}</span> of <span className="text-navy">1,248</span> challenges</p><div className="mt-4 grid gap-4 xl:grid-cols-3 lg:grid-cols-2">{rows.map(c=><ChallengeCard key={c.id} challenge={c}/>)}</div>{!rows.length && <div className="mt-6"><EmptyState/></div>}</div>;
}
