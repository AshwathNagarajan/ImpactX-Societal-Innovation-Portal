...
export default function SubmitChallenge() {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-8"><span className="rounded-full border border-blue/20 bg-blue/10 px-3 py-1 text-sm font-semibold text-blue">Citizen Submission</span><h1 className="mt-4 text-4xl font-semibold tracking-tight text-navy">Submit a Societal Challenge</h1><p className="mt-2 text-slate-600">No login required. Your submission enters AI analysis and government validation.</p>
    <div className="mt-7 overflow-x-auto pb-2"><div className="flex min-w-max gap-3">{steps.map((s,i)=><div key={s} className="flex items-center gap-3 rounded-2xl border bg-[#0d1828] px-4 py-3"><span className={`grid h-8 w-8 place-items-center rounded-xl text-xs font-semibold ${i===0?"impact-gradient text-[#041016]":"bg-slate-100 text-slate-500"}`}>{String(i+1).padStart(2,"0")}</span><span className="text-sm font-medium text-navy">{s}</span></div>)}</div></div>
...
function Panel({ title, children }) { return <section className="premium-surface rounded-2xl border p-5"><h2 className="mb-4 text-2xl font-semibold text-navy">{title}</h2><div className="grid gap-4 md:grid-cols-2">{children}</div></section>; }
...
function Text({ label, v, on, required }) { return <label className="text-sm font-semibold text-slate-600 md:col-span-2">{label}<textarea required={required} value={v} onChange={e=>on(e.target.value)} rows="4" className="mt-1 w-full rounded-lg border px-3 py-2.5 outline-none focus:border-blue"/></label>; }
