...
function Utility({ title, variant }) {
  ...
  return <div><h1 className="text-3xl font-bold text-navy">{title}</h1><div className="mt-5 grid gap-4 lg:grid-cols-3">{rows.map((r,i)=><article key={r} className="rounded-xl border bg-white p-5 shadow-sm"><span className="grid h-10 w-10 place-items-center rounded-lg bg-blue/10 text-blue"><Wrench size={18}/></span><h2 className="mt-4 font-bold text-navy">{r.split(":")[0]}</h2><p className="mt-2 text-sm text-slate-600">{r}</p><button className="mt-4 rounded-lg border px-3 py-2 text-sm font-bold text-blue">Update</button></article>)}</div></div>;
}
