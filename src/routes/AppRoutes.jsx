...
function Utility({ title, variant }) {
  ...
  return <div className="min-w-0"><h1 className="text-2xl font-semibold text-navy md:text-3xl">{title}</h1><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{rows.map((r,i)=><article key={r} className="rounded-xl border bg-white p-5 shadow-sm"><span className="grid h-10 w-10 place-items-center rounded-lg bg-blue/10 text-blue"><Wrench size={18}/></span><h2 className="mt-4 font-semibold text-navy">{r.split(":")[0]}</h2><p className="mt-2 text-sm text-slate-600">{r}</p><button className="mt-4 w-full rounded-lg border px-3 py-2 text-sm font-bold text-blue sm:w-auto">Update</button></article>)}</div></div>;
}
