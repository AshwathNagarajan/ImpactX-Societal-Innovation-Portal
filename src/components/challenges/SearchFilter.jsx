...
export default function SearchFilter({ filters, setFilters }) {
  ...
  return <div className="premium-surface rounded-2xl border p-4 backdrop-blur">
    <div className="grid gap-3 md:grid-cols-6">
      <label className="relative md:col-span-2"><Search className="absolute left-3 top-3 text-blue" size={18}/><input value={filters.search} onChange={(e)=>update("search",e.target.value)} placeholder="Search challenges" className="w-full rounded-xl border py-2.5 pl-10 pr-3 outline-none"/></label>
