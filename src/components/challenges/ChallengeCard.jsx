...
export default function ChallengeCard({ challenge }) {
  return <Link to={`/challenges/${challenge.id}`} className="group block w-full min-w-0 rounded-2xl border bg-[#0d1828] p-5 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-blue/30">
    <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><span className="inline-flex max-w-full truncate rounded-full border border-blue/20 bg-blue/10 px-2.5 py-1 text-xs font-semibold text-blue">{challenge.category}</span><p className="mt-3 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{challenge.id}</p><h3 className="mt-1 text-lg font-semibold leading-tight text-navy md:text-xl">{challenge.title}</h3></div><div className="flex flex-wrap gap-2"><StatusBadge status={challenge.status}/><PriorityBadge priority={challenge.priority}/></div></div>
