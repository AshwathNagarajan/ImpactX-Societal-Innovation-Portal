...
export default function AccountMenu({ title, dark = false }) {
  ...
  return (
    <details className="relative">
      <summary className={`flex list-none items-center gap-3 rounded-xl border px-3 py-2 shadow-sm ${dark ? "border-white/10 bg-white/5 text-slate-100 hover:bg-white/10" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`}>
      ...
      <div className={`absolute right-0 mt-3 w-72 rounded-2xl border p-3 shadow-xl ${dark ? "border-white/10 bg-slate-800 text-slate-100" : "border-slate-200 bg-white text-slate-700"}`}>
