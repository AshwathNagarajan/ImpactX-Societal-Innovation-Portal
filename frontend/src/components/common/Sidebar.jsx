...
export default function Sidebar({ title, items, dark = false }) {
  ...
  const content = <aside className={`flex h-full flex-col border-r ${dark ? "border-white/10 bg-slate-950 text-slate-100" : "border-slate-200 bg-white"}`}>
    ...
    <nav className="scrollbar-thin flex-1 space-y-2 overflow-y-auto p-4">{items.map(({to,label,icon:Icon})=><NavLink key={to} to={to} end={to.split("/").length===2} onClick={()=>setOpen(false)} className={({isActive})=>`relative flex min-h-11 items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium ${dark ? (isActive ? "bg-cyan-300/10 text-cyan-100 before:absolute before:left-0 before:top-2 before:h-7 before:w-0.5 before:rounded-full before:bg-cyan-300" : "text-slate-400 hover:bg-white/5 hover:text-white") : (isActive ? "bg-blue-50 text-blue before:absolute before:left-0 before:top-2 before:h-7 before:w-0.5 before:rounded-full before:bg-blue" : "text-slate-600 hover:bg-slate-50 hover:text-navy")}`}>{Icon && <Icon size={18}/>} {label}</NavLink>)}</nav>
    <div className={`m-4 rounded-2xl border p-4 ${dark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50"}`}>
      <div className="flex items-center gap-3 text-sm"><UserCircle className={dark ? "text-cyan-300" : "text-blue"} size={24}/><div><p className={`font-semibold ${dark ? "text-white" : "text-navy"}`}>Demo User</p><p className={`text-xs ${dark ? "text-slate-400" : "text-slate-500"}`}>{title}</p></div></div>
  ...
  return <>
    <button onClick={()=>setOpen(true)} className={`fixed left-3 top-3 z-50 rounded-xl border p-2.5 shadow-md lg:hidden ${dark ? "border-white/10 bg-slate-950 text-cyan-200" : "border-slate-200 bg-white text-blue"}`}><Menu size={20}/></button>
