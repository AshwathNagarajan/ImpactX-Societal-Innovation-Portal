...
export default function Sidebar({ title, items, dark = false }) {
  ...
  const content = <aside className={`flex h-full flex-col border-r ${dark ? "border-white/10 bg-slate-950 text-slate-100" : "border-slate-200 bg-white"}`}>
    <div className={`border-b p-6 ${dark ? "border-white/10" : "border-slate-200"}`}>
      <div className="flex items-center gap-3"><img src="/impactx-logo.png" alt="IMPACTX" className="h-12 w-12 rounded-xl bg-white object-contain p-1" /><div><h2 className={`text-xl font-semibold tracking-tight ${dark ? "text-white" : "text-navy"}`}>IMPACTX</h2><p className={`text-sm ${dark ? "text-slate-400" : "text-slate-500"}`}>{title}</p></div></div>
    </div>
