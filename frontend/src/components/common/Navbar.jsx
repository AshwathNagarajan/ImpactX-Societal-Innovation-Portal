...
export default function Navbar() {
  ...
  return <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
    <div className="mx-auto flex h-[72px] max-w-[1440px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-10 xl:px-12">
      <Link to="/" onClick={()=>setOpen(false)} className="flex items-center gap-3 text-lg font-semibold tracking-tight text-navy sm:text-xl"><img src="/impactx-logo.png" alt="IMPACTX" className="h-11 w-11 rounded-xl object-contain" /> IMPACTX</Link>
      <nav className="hidden items-center gap-2 lg:flex">{nav.map(([to,label])=><NavLink key={`${to}-${label}`} to={to} end={to === "/"} className={({isActive})=>`relative rounded-xl px-4 py-2.5 text-sm font-medium ${isActive ? "bg-blue-50 text-blue" : "text-slate-500 hover:bg-slate-50 hover:text-navy"}`}>{({isActive}) => <>{label}{isActive && <span className="absolute inset-x-4 -bottom-3 h-0.5 rounded-full impact-gradient"/>}</>}</NavLink>)}</nav>
    ...
    {open && <nav className="border-t border-slate-200 bg-white px-4 py-4 shadow-lg lg:hidden">
      <div className="grid gap-2">{nav.map(([to,label])=><NavLink key={`${to}-${label}-mobile`} to={to} end={to === "/"} onClick={()=>setOpen(false)} className={({isActive})=>`rounded-xl px-4 py-3 text-sm font-medium ${isActive ? "bg-blue-50 text-blue" : "text-slate-600 hover:bg-slate-50 hover:text-navy"}`}>{label}</NavLink>)}
