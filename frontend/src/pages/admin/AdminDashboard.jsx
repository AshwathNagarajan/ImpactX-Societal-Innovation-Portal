  ...
  return <div className="admin-dark-shell -m-4 rounded-[2rem] p-4 sm:-m-6 sm:p-6 lg:-m-8 lg:p-8">
    ...
    <div className="mt-10 grid gap-8 xl:grid-cols-[1fr_.72fr]">
      <section className="rounded-3xl border border-amber-300/20 bg-slate-800/85 p-6 shadow-[0_20px_50px_rgba(15,23,42,.16)] md:p-7">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-3">{[`${data.challenges.filter(c=>c.priority==="Critical"||c.priority==="High").length} High Priority Challenges`,`${data.challenges.filter(c=>["Submitted","Under Review"].includes(c.status)).length} Awaiting Validation`,`${data.projects.filter(p=>getLifecycleProgress(p)<70).length} Projects Delayed`].map((x,i)=><div key={x} className={`rounded-2xl border border-white/10 bg-white/5 p-5 ${i===0?"text-rose-300":"text-amber-300"}`}><p className="text-3xl font-semibold">{x.split(" ")[0]}</p><p className="mt-2 text-sm text-slate-300">{x.replace(x.split(" ")[0],"")}</p></div>)}</div>
      </section>
      <ImpactScore/>
    </div>
