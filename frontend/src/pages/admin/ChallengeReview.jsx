...
export default function ChallengeReview(){
  ...
  const runAnalysis=async()=>{setBusy(true);try{const res=await aiService.analyzeChallenge(id);setAnalysis(res.data)}catch{setAnalysis(fallbackAnalysis(c))}finally{setBusy(false)}};
  const activeAnalysis=analysis&&Object.keys(analysis).length?analysis:fallbackAnalysis(c);
    ...
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px]">
      ...
      <aside className="min-w-0 space-y-5 lg:sticky lg:top-28 lg:self-start">
        <section className="rounded-2xl border bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 grid gap-3">{["Validate Challenge","Reject","Request More Information","Change Category","Override Severity","Assign Institute"].map((b,i)=><button key={b} className={`min-h-11 rounded-xl px-4 py-2.5 text-sm font-semibold ${i===0?"bg-green text-white":i===1?"bg-red-50 text-red-600":"border border-slate-200 bg-white text-slate-700"}`}>{b}</button>)}</div>
        </section>
        <section className="rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50 via-white to-violet-50 p-6 shadow-sm">
          ...
          <div className="mt-5 space-y-4">{industries.slice(0,2).map(i=><IndustryCard key={i.name} industry={i}/>)}</div>
        </section>
      </aside>
    </div>
  </div>
