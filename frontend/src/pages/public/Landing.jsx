...
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip, LineChart, Line, PieChart, Pie, Cell, RadarChart, PolarGrid, PolarAngleAxis, Radar, Legend } from "recharts";
  ...
  return <div>
    ...
    <section className="mx-auto max-w-[1440px] px-4 py-12 sm:px-6 md:py-16 lg:px-10 xl:px-12">
      ...
      <div className="mt-12 grid gap-8 xl:grid-cols-2">
        <ChartCard title="Challenges by Category"><ResponsiveContainer><BarChart data={chartData.category}><XAxis dataKey="name" hide/><YAxis/><Tooltip/><Legend wrapperStyle={{ color: "#cbd5e1", fontSize: 12 }}/><Bar dataKey="value" name="Challenges" fill="transparent" stroke="#22d3ee" strokeWidth={2} radius={[6,6,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Monthly Challenge Submissions"><ResponsiveContainer><LineChart data={chartData.monthly}><XAxis dataKey="month"/><YAxis/><Tooltip/><Legend wrapperStyle={{ color: "#cbd5e1", fontSize: 12 }}/><Line dataKey="submissions" name="Submissions" stroke="#2563EB" strokeWidth={3}/><Line dataKey="resolved" name="Resolved" stroke="#0D9488" strokeWidth={3}/></LineChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Challenge Status Distribution"><ResponsiveContainer><PieChart><Pie data={chartData.status} dataKey="value" nameKey="name" innerRadius={58} outerRadius={90}>{chartData.status.map((_,i)=><Cell key={i} fill={colors[i%colors.length]}/>)}</Pie><Tooltip/><Legend layout="vertical" align="right" verticalAlign="middle" iconType="circle" wrapperStyle={{ color: "#cbd5e1", fontSize: 12, lineHeight: "22px", paddingLeft: 16 }}/></PieChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Challenges by District"><ResponsiveContainer><BarChart data={chartData.district} layout="vertical" margin={{ left: 30 }}><XAxis type="number"/><YAxis dataKey="name" type="category"/><Tooltip/><Legend wrapperStyle={{ color: "#cbd5e1", fontSize: 12 }}/><Bar dataKey="challenges" name="Challenges" fill="transparent" stroke="#34d399" strokeWidth={2} radius={[0,6,6,0]}/></BarChart></ResponsiveContainer></ChartCard>
        <ChartCard title="University vs Industry Participation"><ResponsiveContainer><BarChart data={chartData.participation}><XAxis dataKey="name"/><YAxis/><Tooltip/><Legend wrapperStyle={{ color: "#cbd5e1", fontSize: 12 }}/><Bar dataKey="institutes" name="Institutes" fill="transparent" stroke="#22d3ee" strokeWidth={2}/><Bar dataKey="industries" name="Industries" fill="transparent" stroke="#a78bfa" strokeWidth={2}/></BarChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Impact by Sector"><ResponsiveContainer><RadarChart data={chartData.impact}><PolarGrid/><PolarAngleAxis dataKey="sector"/><Tooltip/><Legend wrapperStyle={{ color: "#cbd5e1", fontSize: 12 }}/><Radar dataKey="impact" name="Impact Score" fill="#D97706" fillOpacity={0.25} stroke="#D97706"/></RadarChart></ResponsiveContainer></ChartCard>
      </div>
