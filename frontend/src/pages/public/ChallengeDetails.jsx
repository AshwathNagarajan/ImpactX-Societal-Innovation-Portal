...
export default function ChallengeDetails() {
  ...
  const isAssignedToInstitute = isInstitute && display(c.status).toLowerCase() === "assigned";
...
function toUi(doc) { return { ...doc, id: doc.challenge_id, subCategory: doc.subcategory, city: doc.city_or_village, date: String(doc.created_at || "").slice(0,10), status: display(doc.status), priority: display(doc.priority), affected: doc.people_affected || 0, institutes: doc.matched_institutes?.length || 0, industries: doc.industry_partners?.length || 0, submitter: `${doc.submitted_by?.name || "Citizen"}, ${doc.submitted_by?.type || "Citizen"}` }; }
